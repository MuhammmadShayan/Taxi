(function(){
  async function refreshAuth() {
    try {
      ...
      if (!user) {
        el.innerHTML = '\n          <a href="#" class="theme-btn theme-btn-small theme-btn-transparent me-1" data-bs-toggle="modal" data-bs-target="#signupPopupForm">Sign Up<\/a>\n          <a href="#" class="theme-btn theme-btn-small" data-bs-toggle="modal" data-bs-target="#loginPopupForm">Login<\/a>\n        ';
      } else {
        const dashboard = user.user_type === 'admin' ? '/admin' : (user.user_type === 'driver' ? '/driver' : '/user');
        el.innerHTML = `
          <span class="me-2">${user.first_name || ''} ${user.last_name || ''} (${user.user_type})</span>
          <a href="${dashboard}" class="theme-btn theme-btn-small theme-btn-transparent me-1">Dashboard</a>
          <button id="logout-btn" class="theme-btn theme-btn-small">Logout</button>
        `;
        ...
        if (btn) {
          btn.addEventListener('click', async () => {
            try {
              ...
              location.reload(); // Still reload on error
            }
          });
        } else {
